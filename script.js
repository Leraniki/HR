...
function initHrPanelPage() { const chatWindow = document.getElementById('chat-window'); const chatInput = document.getElementById('chat-input'); const sendBtn = document.getElementById('chat-send-btn'); const resultsContainer = document.getElementById('search-results-container'); const clearChatBtn = document.getElementById('clear-chat-btn'); let currentSearchQuery = {}; let hrUser = DB.getCurrentUser(); const addMessage = (text, sender, persist = true) => { const message = document.createElement('div'); message.className = `flex ${sender === 'user' ? 'justify-end' : ''}`; message.innerHTML = `<div class="${sender === 'user' ? 'bg-accent text-white' : 'bg-gray-100'} rounded-lg p-3 max-w-lg"><p>${text}</p></div>`; chatWindow.appendChild(message); chatWindow.scrollTop = chatWindow.scrollHeight; if (persist) { if (!hrUser.chatHistory) hrUser.chatHistory = []; hrUser.chatHistory.push({ sender, text }); DB.setCurrentUser(hrUser); } }; const loadChatHistory = () => { chatWindow.innerHTML = ''; if (hrUser.chatHistory && hrUser.chatHistory.length > 0) { hrUser.chatHistory.forEach(msg => addMessage(msg.text, msg.sender, false)); } }; function parseHrQuery(query) { query = query.toLowerCase(); const parsed = { position: 'Новая должность', project: 'Внутренний проект', details: `Полное описание вакансии на основе вашего запроса:\n"${query}"` }; const positionMatch = query.match(/(?:a|an)?\s*(senior|junior|middle|lead)?\s*([\w\s]+?)\s*(developer|engineer|manager|lead)/); if (positionMatch) { parsed.position = positionMatch[0].trim().replace(/\b\w/g, l => l.toUpperCase()); } const projectMatch = query.match(/for project\s+(['"]?)([\w\s]+)\1/); if (projectMatch) { parsed.project = projectMatch[2].trim().replace(/\b\w/g, l => l.toUpperCase()); } const skillsMatch = query.match(/(?:needs|with skills|requiring)\s+([\w\s,]+)/); if (skillsMatch) { parsed.details += `\n\nКлючевые требования: ${skillsMatch[1].trim()}`; } return parsed; } const renderResults = (employee) => { resultsContainer.innerHTML = `<div class="p-4 rounded-xl border flex items-center justify-between group hover:bg-blue-50 transition-colors"><div class="flex items-center gap-4"><img class="h-12 w-12 rounded-full object-cover" src="${employee.avatar}" alt="Candidate"><div><p class="font-bold">${employee.name}</p><p class="text-sm text-gray-500">${employee.role}</p></div></div><div class="flex items-center gap-2"><button class="view-profile-btn px-4 py-2 text-sm rounded-xl bg-secondary-bg font-bold">View Profile</button><button class="send-offer-btn px-4 py-2 text-sm rounded-xl bg-accent text-white font-bold">Send Offer</button></div></div>`; document.querySelector('.view-profile-btn').addEventListener('click', () => { sessionStorage.setItem('viewingAsHR', 'true'); window.location.href = 'profile.html'; }); document.querySelector('.send-offer-btn').addEventListener('click', () => { sendOfferToEmployee(currentSearchQuery.position, currentSearchQuery.project, currentSearchQuery.details); }); }; const performSearch = (query) => { currentSearchQuery = parseHrQuery(query); setTimeout(() => { const employee = DB.getEmployeeData(); const keywords = query.toLowerCase().split(/\s*,\s*|\s+/); const employeeSkills = employee.skills.map(s => s.name.toLowerCase()); const isMatch = keywords.some(kw => employeeSkills.includes(kw) || employee.role.toLowerCase().includes(kw)); if (isMatch) { addMessage(`Нашла подходящего кандидата для позиции "${currentSearchQuery.position}".`, 'ai'); renderResults(employee); sessionStorage.setItem('lastHrSearchQuery', query); } else { addMessage('Извините, по вашему запросу никого не найдено. Попробуйте уточнить требования.', 'ai'); resultsContainer.innerHTML = `<div class="text-center py-20 text-gray-500"><p>No candidates found...</p></div>`; sessionStorage.removeItem('lastHrSearchQuery'); } }, 500); }; const handleSend = () => { const query = chatInput.value.trim(); if (!query) return; addMessage(query, 'user'); chatInput.value = ''; performSearch(query); }; clearChatBtn.addEventListener('click', () => { hrUser.chatHistory = [ { sender: 'ai', text: 'Hello! I\'m here to help you. Try asking: "Find a senior python developer for project "Phoenix" with skills Docker, Django"' } ]; DB.setCurrentUser(hrUser); loadChatHistory(); resultsContainer.innerHTML = `<div class="text-center py-20 text-gray-500"><p>Ask AI Assistant to find candidates</p></div>`; sessionStorage.removeItem('lastHrSearchQuery'); }); sendBtn.addEventListener('click', handleSend); chatInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') handleSend(); }); loadChatHistory(); const savedQuery = sessionStorage.getItem('lastHrSearchQuery'); if (savedQuery) { currentSearchQuery = parseHrQuery(savedQuery); renderResults(DB.getEmployeeData()); } }
